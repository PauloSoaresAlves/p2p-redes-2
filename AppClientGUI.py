...
def requerer_lista_arquivos(server_socket: socket.socket, listener_port: str, client_instance: client_persist):
    ...
    while True:
        ...
        if eventos == sg.WIN_CLOSED:
            ...
            requerer_arquivo(table[valores['-TABLE-'][0]],listener_port, client_instance)
            sg.popup(f"Now playing: {table[valores['-TABLE-'][0]][1]}")
        elif eventos == "Pausar":
            if client_instance.paused:
                client_instance.paused = False
                btnPausar.update("Pausar")

            elif client_instance.paused == False:
                client_instance.paused = True
                btnPausar.update("Retomar")
...
def recieve_audio(my_address, my_port,client_instance: client_persist):
    ...
    client_socket.setsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF,CHUNK)
    ...
    while True:
        ...
        if decoded_data == "Stop Streaming":
            ...
            client_instance.notPlaying = False
...
def listen_client(my_address, my_port,client_instance: client_persist):
    ...
    while True:
        ...
        if filename not in os.listdir(f'./shared_{client_instance.id}'):
            ...
            client_instance.streaming = True
            ...
            my_port_string = str(my_port)
            client_server.sendto(json.dumps({'channels':channels,'sample_rate':sample_rate,'sender_port':my_port_string[1:]}).encode(),(addr[0], int(reciever_port)))
            print(channels,chunk_size,channels,my_port_string[1:])
            while client_instance.streaming:
    ...
    if server_socket != None:
        ...
        while conectado:
            ...
            if eventos == sg.WIN_CLOSED:
                ...
                window.close()
                requerer_lista_arquivos(server_socket,listener_port,client_instance)
